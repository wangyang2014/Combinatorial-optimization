/* Program: an implementation of the iterated tabu search (ITS) algorithm 
   ...
   Examples of invocation:
     ...
     strcpy(in_file_name,"D:\\data\\RanInt_n012_ds_06.txt");
     strcpy(out_file_name,"D:\\temp\\RanInt_n012_ds_06.res");
...
#include "grouping_its.h"
 ...
 void check_best_solution(FILE *out,int n,int gc,long best_value,
      Instance *pinst,Solution *psol,Group *pgr)
 {int i,j,k;
  int count;
  long value_obj=0,value_groups=0;
  ...
  if (value_obj!=best_value)
     {fprintf(out,
          "!!! some discrepancy in ITS solution values (1): %10ld   %10ld\n",
  ...
  if (value_groups!=best_value)
     {fprintf(out,
          "!!! some discrepancy in ITS solution values (2): %10ld   %10ld\n",
 ...
 long random_start(int n,int gc,double coef,double *seed,
      Instance *pinst,Solution *psol,Group *pgr)
 {int i,j,k,m;
  int r=0;
  int kg,mg;
...
// Calculating the value of the objective function
 ...
 long get_perturb_solution(int n,int gc,int perturb_count,
      int cand_list_size,int adv_use_rel,long sol_value,double coef,
...
// Performing exchange (or relocation)
 ...
 long local_search(int n,int gc,long best_value,double coef,
      double *seed,Instance *pinst,Solution *psol,Group *pgr)
...
// Trying to relocate the element under consideration
             {for (j0=1;j0<=gc;j0++)
                 {j=(psol+j0)->pg;if (j==kg || (pgr+j)->sz>=(pgr+j)->maxs) continue;
                  dif=con(k,j)-con(k,kg);
                  if (dif>0) {temp_best=dif;ind1=k;ind2=-j;break;}
                 } // for j0
              if (temp_best>0) break;
             }
...
// If "dif>0", then better solution has been found
              if (dif>0) {temp_best=dif;ind1=k;ind2=m;break;}
             } // for m0
          if (temp_best>0) break;
         } // for k0
      if (temp_best>0)
...
// Solution value is updated
 ...
 long tabu_search(int n,int gc,int keep_tabu_time,int start,
      long sol_value,int it_bound,long time_limit,double coef,
      long *best_value,int *stop_cond,int *time_values_opt,
...
// Tabu tenure values are set to 0
  ...
  while (it<it_bound)
     {it++;imp=0;
      ind1=ind2=-1;temp_best=NEG_LARGE;val_dif=*best_value-sol_value;
      for (k=1;k<=n;k++)
         {kg=(psol+k)->s;
// Looking for the best pairwise exchange of elements 
...
// Checking tabu search termination rule
 ...
 long iterated_tabu_search(int n,int gc,int keep_tabu_time,
  ...
  if (adv_use_rel>0)
     {for (i=2,max_val=-1;i<=n;i++) if (d(1,i)>max_val) {max_val=d(1,i);s=i;}
      for (i=2;i<=n;i++)
         {if (i==s) continue;
          sum=d(1,i)+d(s,i);
          if (sum<max_val) {adv_use_rel=0;break;}
         }
     }
...
// that contains the dissimilarity matrix
  ...
  for (i=0;i<=n;i++) ALI((pinst+i)->d,n+1)
  for (i=0;i<=n;i++) ALI((pinst+i)->con,gc+1)
...
// regarding the performance of the algorithm
  ...
  fprintf(out,"   last improvement at TS start no.          = %3ld\n",
      (psol+2)->perf);
  fprintf(out,"   value found by ITS                        = %8ld\n",
      (psol+6)->perf);
