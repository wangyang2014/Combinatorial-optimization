...
void InitiaPoP(Solution *PP)
{
     ...
     { 
       ...
       if(PP[i].value > SS_BEST.value)
        ...
        {  
             if(N_times==0) total_time_suc  += (double)((clock()- starting_time)/CLOCKS_PER_SEC);
             N_times++; 
        } 
...
void Outresulting(char *filename,char *outfile, double time)
{
    ...
    fp=fopen(buff,"a+");
    fprintf(fp,"%s  %d   %lf   %lf  %d   %lf \n", filename, N, BestResult, AvgResult, Nhit,time);  
	fclose(fp);         
    ...
    // int N_times=0;
     ...
     do{
          ...
          { 
              ...
              One_Flip_Tabu_Search(S_off.S, &S_off.value, alpha); //printf("\n c  f = %lf \n",S_off.value);
