...
void grouping_its(char *,char *,double,long,Results *);
 ...
 {FILE *out;
  ...
  int count;
  long time_limit;
  long max_value=-1,best_known_value=0;
  long dif_best;
  ...
  strcpy(param_string,argv[4]);time_limit=atol(param_string);
  strcpy(param_string,argv[5]);count=atoi(param_string);
  if (argc==7)
     {strcpy(bkv_string,argv[6]);best_known_value=atol(bkv_string);}
  ...
  for (i=1;i<=count;i++)
     {strcpy(out_file_name,out_file_name_init);
      ...
      grouping_its(in_file_name,out_file_name,seeds[i],time_limit,pres);
  ...
  fprintf(out,"maximum value over all runs = %8ld \n",max_value);
  if (argc==7)
