...
/******************************************************************************************/
/**********    5.   Variable Neighborhood Desecent method (EVND)     **********************/
/******************************************************************************************/
void BuildNeighbors()
{
     int i,j,g;
...
/******************************************************************************************/
/******************** 7. Tabu Search Method based on 2-1 exchange move    ****************/
/******************************************************************************************/
void Swap_Move_Tabu_Search(int p_into[], double SizeGroup[], double *value, double *PrevTime)
{
	int i, j, v, g;
	int num_tabu_best, num_best ;  // the number of tabu neighbors and non-tabu neighbors
	int x, y, z;
	int best_x[ 50 ], best_y[ 50 ], best_z[ 50 ];
	int tabu_best_x[ 50 ], tabu_best_y[ 50 ], tabu_best_z[50];
	int iter, select, num; 
	double tabu_best_delta, best_delta, delt ;
	int old_group, old_group1, old_group2, swap;

	for(i=0;i<N;i++) p[i] = p_into[i];
	starting_time = clock();
	Build_Delta_Matrix();
	f_best = f; 
	//printf("\n\n");
	//cout << endl << "Swap_Move :       iter       f       f_best      time " << endl;
	//cout << "---------------------------------------------------" << endl; 
    for(i=0;i<K;i++) SizeG[ i ] = SizeGroup[i];
    for(i=0;i<N;i++) bestp[ i ] = p_into[i];
	iter = 0 ; 
	while( 1.0*(clock() - starting_time )/CLOCKS_PER_SEC < Time_limit - *PrevTime )
	{
		num = 0 ;
		...
		for( x = 0 ; x < N ; x++ )
            for( y = x+1 ; y < N ; y++ )
               for(z = 0; z < N; z ++)
					if( (p[x] == p[y] && p[x] != p[z]) && (SizeG[p[x]] - w[y] - w[x] + w[z] >= LB[p[x]]) && (SizeG[p[x]] - w[y] - w[x] + w[z] <= UB[p[x]]) && (SizeG[p[z]] + w[x] + w[y] - w[z] >= LB[p[z]]) && (SizeG[p[z]] + w[x] + w[y] - w[z] <= UB[p[z]]) )
                    {
						num ++ ;
					    delt =  (Delta_Matrix[ x ][ p[z] ] - Delta_Matrix[ x ][ p[ x ] ]) + (Delta_Matrix[ z ][ p[x] ] - Delta_Matrix[ z ][ p[ z ] ]) - DT[x][z];
                        delt += ((Delta_Matrix[ y ][ p[z] ] - Delta_Matrix[ y ][ p[ y ] ]) + (DT[y][x]- DT[y][z]));
						
						if( (TabuTenure[ x ][ p[z] ] <= iter) && (TabuTenure[ y ][ p[z] ] <= iter) && (TabuTenure[ z ][ p[x] ] <= iter) ) // if this is not tabued 
                        {
							if( delt > best_delta )
							{
								best_x[ 0 ] = x ; 
						...
						else if( (TabuTenure[ x ][ p[z] ] > iter) || (TabuTenure[ y ][ p[z] ] > iter)||(TabuTenure[ z ][ p[x] ] > iter))// if it is tabu 
						{ 
			...
			if( ( num_tabu_best > 0 && tabu_best_delta > best_delta && ( f + tabu_best_delta > f_best ) ) || (num_best == 0) )  // aspiration criterion 
			{
                ...
                old_group  = p[ tabu_best_x[ select ] ] ; 
                old_group1 = p[ tabu_best_y[ select ] ] ;  
                old_group2 = p[ tabu_best_z[ select ] ] ;  
                
                SizeG[old_group]  += (w[tabu_best_z[ select ]] - w[tabu_best_y[ select ]] - w[tabu_best_x[ select ]]);
                SizeG[old_group2] += (w[tabu_best_x[ select ]] + w[tabu_best_y[ select ]] - w[tabu_best_z[ select ]]);  
                      
               	One_Move_Update_Delta_Matrix1( tabu_best_x[ select ], old_group, old_group2);
				swap   =  p[ tabu_best_x[ select ] ];
				p[ tabu_best_x[ select ] ] = p[ tabu_best_z[ select ] ];
				One_Move_Update_Delta_Matrix1( tabu_best_z[ select ], old_group2, old_group);
				p[ tabu_best_z[ select ] ] = swap;      
                                            
                One_Move_Update_Delta_Matrix1( tabu_best_y[ select ], old_group1, old_group2);
                p[tabu_best_y[ select ]] = old_group2;      			
				
				TabuTenure[ tabu_best_x[ select ] ][ old_group ]  =   5  ; 
				TabuTenure[ tabu_best_x[ select ] ][ old_group ] += iter ; 
                            
				TabuTenure[ tabu_best_y[ select ] ][ old_group1 ] =   5  ; 
				TabuTenure[ tabu_best_y[ select ] ][ old_group1 ] += iter ; 
                
               	TabuTenure[ tabu_best_z[ select ] ][ old_group2 ] =   5  ; 
				TabuTenure[ tabu_best_z[ select ] ][ old_group2 ] += iter ; 
               // printf("tabu delt = %lf\n",tabu_best_delta );			
	    	} 
			...
			else //choose the best non tabu move
			{
				...
				select = rand( ) % num_best ;  
       		    ...
       		    old_group  = p[ best_x[ select ] ] ; 
                old_group1 = p[ best_y[ select ] ] ;  
                old_group2 = p[ best_z[ select ] ] ;  
                 
                SizeG[old_group]  += (w[best_z[ select ]] - w[best_y[ select ]] - w[best_x[ select ]]);
                SizeG[old_group2] += (w[best_x[ select ]] + w[best_y[ select ]] - w[best_z[ select ]]);  
                          
               	One_Move_Update_Delta_Matrix1(best_x[ select ], old_group, old_group2);
				swap   =  p[ best_x[ select ] ];
				p[ best_x[ select ] ] = p[ best_z[ select ] ];
				One_Move_Update_Delta_Matrix1( best_z[ select ], old_group2, old_group);
				p[ best_z[ select ] ] = swap;      
                                            
                One_Move_Update_Delta_Matrix1( best_y[ select ], old_group1, old_group2);
                p[best_y[ select ]] = old_group2; 
				
				TabuTenure[ best_x[ select ] ][ old_group ] =   5  ; 
				TabuTenure[ best_x[ select ] ][ old_group ] += iter ; 
                            
				TabuTenure[ best_y[ select ] ][ old_group1 ] =  5  ; 
				TabuTenure[ best_y[ select ] ][ old_group1 ] += iter ; 
				
				TabuTenure[ best_z[ select ] ][ old_group2 ] =  5  ; 
				TabuTenure[ best_z[ select ] ][ old_group2 ] += iter ; 
			//	printf(" delt = %lf \n",best_delta );
			} 	
			iter ++ ;           	
			if( f > f_best + 1.0e-10 )
			{
			   f_best = f;
			   for( i = 0 ; i < N ; i ++ ) bestp[ i ] = p[ i ] ;
			   for( g = 0 ; g < K ; g ++ ) SizeGroup[g] = SizeG[g] ;
			   total_time = *PrevTime + ( clock() - starting_time )/CLOCKS_PER_SEC ;
			   //printf("Swap_Move :  %8d  %5.6lf   %5.6lf   %5.3lf  %5.3lf  s\n", iter, f, f_best, total_time,*PrevTime);
			}    
     }
    *value = f_best; 
   
    for(i=0;i<N;i++) p_into[i] = bestp[i];
    for(i=0;i<K;i++) SizeGroup[i] = 0; 
    for(i=0;i<N;i++) SizeGroup[p_into[i]] += w[i]; 
}
...
void GRASP_TS()
{
    ...
    starting_time = clock();
    GS.cost = -9999999; 
    while(1.0*(clock()- starting_time)/CLOCKS_PER_SEC  <  Time_limit/2)
    {  
      InitialSol(CS); 
    ...
    for(i=0;i<N;i++) CS.p[i] = GS.p[i]; 
      for(j=0;j<K;j++) CS.SizeG[j] = GS.SizeG[j]; 
    CS.cost = GS.cost;   
    
    ctime = Time_limit/2;
    Swap_Move_Tabu_Search(CS.p, CS.SizeG, &CS.cost, &ctime);
    if(CS.cost > GS.cost + 1.0e-10) 
    {
       for(i=0;i<N;i++) GS.p[i] = CS.p[i]; 
       for(j=0;j<K;j++) GS.SizeG[j] = CS.SizeG[j]; 
       GS.cost = CS.cost;         
...
/******************************************************************************************/
/********************    9. Main Function for Multiple Runs     **************************/
/******************************************************************************************/
