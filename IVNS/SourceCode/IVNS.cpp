             ...
             for(z=0;z<N;z++)
             {
              ...
              // delt = (Delta[ x ][ p[z] ] + Delta[ z ][ p[x] ] - DT[x][z]) + Delta[ y ][ p[z] ] + DT[y][x]- DT[y][z];  
               ...
               if(delt > 1.0e-10)
               {
				...
				One_Move_Update_Delta_Matrix1( x, old_g, old_g2);
				
				swap   = p1[ x ];
				p1[ x ] = p1[ z ];
				One_Move_Update_Delta_Matrix1( z, old_g2, old_g);
				p1[ z ] = swap; 
                  
                One_Move_Update_Delta_Matrix1( y, old_g1, old_g2);
                p1[y] = old_g2;
...
void RandomShake(int L, int p1[], double SizeGroup[])
{
	...
	{     
        ...
        if(Neighbors[cur_index].type == 1 ) 
        {
             ...
             if( ( p1[v] != g ) && ( SizeGroup[ p1[v] ] - w[v] >= LB[ p1[v] ] ) && (SizeGroup[g] + w[v] <= UB[g])  )
             { 
               old_g = p1[v] ;              
        ...
        else if(Neighbors[cur_index].type == 2 )  
        {
             ...
             if( (p1[x] != p1[y]) && (SizeGroup[ p1[x] ] + (w[y]-w[x]) >= LB[p1[x]]) && (SizeGroup[ p1[x] ] + (w[y]-w[x]) <= UB[p1[x]]) && (SizeGroup[ p1[y] ] + (w[x]-w[y]) >= LB[p1[y]]) && (SizeGroup[ p1[y] ] + (w[x]-w[y]) <= UB[p1[y]]) )
             { 
                old_g  = p1[ x ]; 
                old_g1 = p1[ y ];    
...
{
     RandomInitiaSol(S.p, S.SizeG); //Proof(S);
}

/******************************************************************************************/
/**********************     8. Maxima Search  Procedure     *******************************/
/******************************************************************************************/
void MaximaSearch(int partition[], double SizeGroup[], double *cost, double *PrevTime)
{   
    int i, j, L ;  
    int counter; 
    double delt, f_c;
	...
	*cost = f;
	L = int (0.02*N);
	if(L<5) L = 5;
	L = L > 15 ? 15: L;  // L = min{ max{5,0.02*N}, 15 }
	counter = 0; 
    do
    {   
       f_c = *cost; 
       ...
       RandomShake(L, p, SizeG);  
       Extend_VND(p, SizeG, &f_c);
       ...
       {    
            *cost = f_c;
            ...
            counter = 0;  
           // printf("f =%lf VND imrptove !\n",f_c);
       }   
       else counter ++;    
       if(f_c > GS.cost)
       {
         for(i=0;i<N;i++) GS.p[i] = p[i]; 
         for(j=0;j<K;j++) GS.SizeG[j] = SizeG[j]; 
         GS.cost = f_c;  
         total_time = ( clock() - starting_time )/CLOCKS_PER_SEC ; 
       }
    }while( (counter < 30) && (1.0*(clock()- ctime)/CLOCKS_PER_SEC  <  Time_limit - *PrevTime) );   
} // for the intensification search

/******************************************************************************************/
/*****************      9. Iterated VNS (IVNS) Algorithm           ************************/
/******************************************************************************************/
void IVNS()
{   
    int i,j;
   ...
   // GS.cost = CS.cost; 
    ...
    {  
      ...
      RandomShake(L, CS.p, CS.SizeG);
      time = 1.0*(clock()- starting_time)/CLOCKS_PER_SEC ;
      MaximaSearch(CS.p, CS.SizeG, &CS.cost, &time); 
    }    	
}
...
int main(int argc, char *argv[])
{
    ...
    int seed; 
    double Sum;
    const int  Times = 20; // The times of runs
   ...
   // Solution_File = "RanReal240_01.txt.sol";
	...
	//max_beta = atoi(argv[3]);
	//Time_limit = atoi(argv[4]); 
    Output_File_Name = "results.txt";
    Solution_File = "best_Sol.txt"; 
