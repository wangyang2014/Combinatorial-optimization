...
// It is sorted in nonincreasing we(.) order
 ...
 {int i,j,k;
  ...
  if (value_obj!=best_value)
     {fprintf(out,
          "!!! some discrepancy in GA solution values (1): %10ld   %10ld\n",
  ...
  if (value_groups!=best_value)
     {fprintf(out,
          "!!! some discrepancy in GA solution values (2): %10ld   %10ld\n",
...
void preparation_for_local_search(int n,int gc,Instance *pinst,
 ...
 long local_search(int n,int gc,int prepar_req,long best_value,
...
// Solution value is updated
 ...
 void store_new_best_sol(int n,int gc,long new_value,
      long offspr_no,int *time_values_opt,
      long *best_value,double *time_to_opt,
 ...
 void append_to_population(int n,int gc,int memb_count,long sol_value,
      int *time_values_opt,long *best_value,double *time_to_opt,
      clock_t start_time,Solution *psol,Group *pgr)
 {int i,j;
  int ln;
 ...
 long get_offspring(int n,int gc,int maxsize,
 ...
 int evaluate_offspring(int n,int gc,int psize,int psize_req,
      long offspr_value,long offspr_no,int *time_values_opt,
      long *best_value,double *time_to_opt,
      clock_t start_time,Solution *psol,Group *pgr)
 {int i,j;
  int ind,replaced=0;
 ...
 long genetic_algorithm(FILE *out,int n,int gc,int maxsize,
...
// ST_COUNT is the length of performance statistics array 'perf'
  ...
  ALL(psol->pfv,POPULATION_SIZE+1)
...
// regarding the performance of the algorithm
  ...
  if ((psol+2)->perf<0)
      ...
      fprintf(out,"   last improvement is due offspring no.     = %8ld\n",
          (psol+2)->perf);
  fprintf(out,"   value of the best solution                = %8ld\n",
      (psol+6)->perf);
