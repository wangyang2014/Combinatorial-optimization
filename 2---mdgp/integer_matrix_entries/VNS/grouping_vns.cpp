...
#include "grouping_vns.h"
 ...
 {int i,j,k;
  ...
  if (value_obj!=best_value)
     {fprintf(out,
          "!!! some discrepancy in VNS solution values (1): %10ld   %10ld\n",
  ...
  if (value_groups!=best_value)
     {fprintf(out,
          "!!! some discrepancy in VNS solution values (2): %10ld   %10ld\n",
...
// Calculating the value of the objective function
 ...
 long shake(int n,int gc,int dist,long sol_value,double coef,
...
// Solution value is updated
 ...
 long variable_neighborhood_search(int n,int gc,int dist_min,
...
// that contains the dissimilarity matrix
  ...
  for (i=0;i<=n;i++) ALI((pinst+i)->bcon,gc+1)
